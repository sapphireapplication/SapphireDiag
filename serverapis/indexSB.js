...
const conn = mysql.createConnection({
  host: "localhost",
  user: "root",
  password: "sql123", //for local seema's machine
  //password: "Mysql@123", //for pipeline server machine
//  database: "custd008sb", //"cdemd014",
  database: "mvxd008",
});
app.use(cors());
conn.connect((err) => {
...
app.get("/DataUsageDiagram/Program/:pgmid", async (req, res) => {
  ...
  let query = await conn.query(sql, async (err, results) => {
    ...
    //let sql1 = `SELECT trim(ENTID) AS ENTID, trim(SHORTNM) AS SHORTNM, trim(FTXT) as FTXT FROM ENTSCHEMA AS A WHERE A.ENTID in (${arrent})`;
    //let sql1 = `SELECT trim(ENTID) AS ENTID, trim(SHORTNM) AS ID, trim(FTXT) as TEXT FROM ENTSCHEMA AS A WHERE A.ENTID in (${arrent})`;

    //console.log("sql1===", sql1);
    let query1 = await conn.query(`SELECT trim(ENTID) AS ENTID, trim(SHORTNM) AS ID, trim(FTXT) as TEXT FROM ENTSCHEMA AS A WHERE A.ENTID in (${arrent})`, async (err1, results1) => {
      if (err1) throw err1;
      console.log("RESULTS in DFD==", results1);
      entschema = results1;

      //let sql2 = `SELECT LINENUM as ID, STN as TEXT, FILENM as FILENM, MVARDB as MVARDB FROM PGMCODE AS A WHERE A.FILENM in (${arrent}) AND A.PGMID='${pgmid}'`;
      let sql2 = `SELECT LINENUM as ID, STN as TEXT, FILENM as FILENM, MVAR as MVARDB, SVAR1 as SVAR1, SVAR2 as SVAR2, SVAR3 as SVAR3, SVAR4 as SVAR4 FROM PGMCODE AS A WHERE A.FILENM in (${arrent}) AND A.PGMID='${pgmid}'`;
      console.log("sql1==", `SELECT trim(ENTID) AS ENTID, trim(SHORTNM) AS ID, trim(FTXT) as TEXT FROM ENTSCHEMA AS A WHERE A.ENTID in (${arrent})`);
...
app.get("/PgmCode/:entid/:pgmid/:shortnm", async (req, res) => {
  ...
  WHERE trim(pgmid)=trim('${pgmid}') AND trim(FILENM)=trim('${entid}') AND (trim(MVAR)=trim('${shortnm}') || trim(SVAR1)=trim('${shortnm}') || trim(SVAR2)=trim('${shortnm}') ||trim(SVAR3)=trim('${shortnm}') || trim(SVAR4)=trim('${shortnm}'));`;
    console.log("SQL===", sql);
    let query = conn.query(sql, (err, results) => {
      if (err) throw err;
      res.send(JSON.stringify({ status: 200, error: null, response: results }));
...
app.get("/SourceBrowser/:pgmId", async function(req, res) {
  ...
  let sql1 = `SELECT PgmId, StnTyp, LineNum, Stn, LEFT(StnDate, 10) as StnDate, OpCode, OpType, CmpTor, LvlNum, MVar, SVar1, SVar2, SVar3, SVar4, FileNm, InvPrNm, CurPrNm, StateID, RuleID, RLSeq, RuleTyp FROM PgmCode WHERE PgmID = '${pgmId}'`;
